...
class AsyncPermissions(activity: AppCompatActivity) {
    ...
    suspend fun requestAfterRationale(vararg permissions: String): PermissionResult =
        suspendCancellableCoroutine { cont ->
            asyncPermissionsFragment.requestFromRationale(*permissions, cont = cont)
        }
}
...
class AsyncPermissionsFragment : Fragment() {
    ...
    internal fun request(vararg permissions: String,
        ...
        if (shouldShowRequestPermissionsRationale(permissions)) {
            PermissionResult.ShouldShowRationale(permissions.toList()).let(cont::resume)
        } else {
    ...
    internal fun requestFromRationale(vararg permissions: String,
                                      cont: CancellableContinuation<PermissionResult>) {
        queue.offer(permissions, cont)
        requestPermissions(permissions, REQUEST_CODE)
    }
    ...
    companion object {
        ...
        private const val TAG: String = "AsyncPermissionsFragment"
        ...
        fun attach(activity: AppCompatActivity): AsyncPermissionsFragment {
            ...
            return AsyncPermissionsFragment().also {
                fragmentManager.apply {
                    beginTransaction().add(it, TAG).commitNow()
                }
            }
        }

        private fun checkSelfPermissions(context: Context,
                                         permissions: Array<out String>): Boolean =
            permissions.all { checkSelfPermission(context, it) }

        private fun checkSelfPermission(context: Context, permission: String): Boolean = try {
    ...
    private class PermissionsContinuationQueue {
        ...
        private data class Entry(val key: String,
