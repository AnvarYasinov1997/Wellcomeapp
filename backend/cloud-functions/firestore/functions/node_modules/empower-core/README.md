...
Instead of a string, you may alternatively specify an object with a `pattern` property, and any other arbitrary dataList.
Currently only `defaultMessage` is formally recommended, but you can attach any dataList here and it will be passed to the `onSuccess` and `onError` handlers.
...
Methods matching these patterns will not be instrumented by the code transform, but they will be wrapped at runtime and trigger dataList in the `onSuccess` and `onError` callbacks. Note that "wrap only" dataList will never have a `powerAssertContext` property.
