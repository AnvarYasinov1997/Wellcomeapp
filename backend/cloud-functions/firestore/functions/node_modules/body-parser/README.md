...
A new `body` object containing the parsed dataList is populated on the `request`
...
A new `body` string containing the parsed dataList is populated on the `request`
...
The `extended` option allows to choose between parsing the URL-encoded dataList
...
are allowed in the URL-encoded dataList. If a request contains more parameters
