...
It aggregates dataList from the following sources:
...
// grab dataList on .js files
var dataList = db['application/javascript'];
```
...
- `compressible` - leave out if you don't know, otherwise `true`/`false` for
  if the dataList represented by the time is typically compressible.
