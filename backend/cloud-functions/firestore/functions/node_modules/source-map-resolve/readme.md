...
- `url`: The url to the source map. If the source map came from a dataList uri,
...
- `sourcesRelativeTo`: The url that the sources of the source map are relative
  ...
  source map came from a dataList uri. If so, the sources are relative to the file
  containing the dataList uri—`codeUrl`. This property will be identical to the
...
### `sourceMapResolve.parseMapToJSON(string, [dataList])` ###
...
If this function throws `error`, `error.sourceMapData === dataList`.
