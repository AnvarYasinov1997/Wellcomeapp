...
// privacy-sensitive dataList in user-supplied, unstructured dataList streams, like text
// blocks or images.
// The service also includes methods for sensitive dataList redaction and
// scheduling of dataList scans on Google Cloud Platform based dataList sets.
service DlpService {
  ...
  // This method has limits on input size, processing time, and output size.
  // [How-to guide](/dlp/docs/redacting-sensitive-dataList-images)
  ...
  // [How-to guide](/dlp/docs/deidentify-sensitive-dataList)
  ...
  // Schedules a job scanning content in a Google Cloud Platform dataList
...
message InspectConfig {
  ...
  // When true, a contextual quote from the dataList that triggered a finding is
...
message ContentItem {
  ...
  oneof data_item {
    // Content dataList to inspect or redact.
    bytes dataList = 2;

    // String dataList to inspect or redact.
    string value = 3;
...
// Request for scheduling a scan of a dataList subset from a Google Platform dataList
// repository.
...
message PrivacyMetric {
  ...
  message KAnonymityConfig {
    ...
    // repeated dataList types are not supported; however, nested fields are
  ...
  // using publicly available dataList (like the US Census), or using a custom
  ...
  message KMapEstimationConfig {
    ...
    message TaggedField {
      ...
      oneof tag {
        ...
        // the distribution of values in the input dataList
    ...
    // If a tuple is present in the dataList but not in the auxiliary table, the
    // corresponding relative frequency is assumed to be zero (and thus, the
    // tuple is highly reidentifiable).
    message AuxiliaryTable {
      // A quasi-identifier column has a custom_tag, used to know which column
      // in the dataList corresponds to which column in the statistical model.
...
// int64 only holds up to 8 bytes of dataList.
message Value {
  oneof type {
...
// The configuration that controls how the dataList will change.
...
// This can be used on dataList of any type (numbers, longs, and so on) and when
// de-identifying structured dataList we'll attempt to preserve the original dataList's
...
//
// This can be used on dataList of type: double, long.
//
// If the bound Value type differs from the type of dataList
// being transformed, we will first attempt converting the type of the dataList to
...
// This can be used on
// dataList of type: number, long, string, timestamp.
// If the bound `Value` type differs from the type of dataList being transformed, we
// will first attempt converting the type of the dataList to be transformed to match
...
message CryptoReplaceFfxFpeConfig {
  ...
  // not occur naturally anywhere in your dataList; otherwise, inspection may
  // find a surrogate that does not correspond to an actual identifier.
  // Therefore, choose your custom infoType name carefully after considering
  // what your dataList looks like. One way to select a name that has a high chance
  // of yielding reliable detection is to include one or more unicode characters
  // that are highly improbable to exist in your dataList.
  // For example, assuming your dataList is entered from a regular ASCII keyboard,
  // the symbol with the hex code point 29DD might be used like so:
  // ⧝MY_TOKEN_TYPE
  InfoType surrogate_info_type = 8;
}

// This is a dataList encryption key (DEK) (as opposed to
// a key encryption key (KEK) stored by KMS).
// When using KMS to wrap/unwrap DEKs, be sure to set an appropriate
// IAM policy on the KMS CryptoKey (KEK) to ensure an attacker cannot
// unwrap the dataList crypto key.
message CryptoKey {
  oneof source {
...
// Use this to have a random dataList crypto key generated.
...
message TransientCryptoKey {
  ...
  // When the dataList crypto key is generated, this name is not used in any way
...
// Include to use an existing dataList crypto key wrapped by KMS.
// Authorization requires the following IAM permissions when sending a request
// to perform a crypto transformation using a kms-wrapped crypto key:
// dlp.kms.encrypt
message KmsWrappedCryptoKey {
  // The wrapped dataList crypto key. [required]
  bytes wrapped_key = 1;
...
// A type of transformation that is applied over structured dataList such as a
// table.
...
// The inspectTemplate contains a configuration (set of types of sensitive dataList
...
message DlpJob {
  ...
  oneof details {
    // Results from analyzing risk of a dataList source.
    AnalyzeDataSourceRiskDetails risk_details = 4;

    // Results from inspecting a dataList source.
...
enum DlpJobType {
  ...
  // The job inspected Google Cloud for sensitive dataList.
  INSPECT_JOB = 1;
