...
// A collection of user dataList indexed by row, column, and timestamp.
...
message Table {
  ...
  //
  // The state of a table's dataList in a particular cluster.
  message ClusterState {
    ...
    enum ReplicationState {
      ...
      // over pre-existing dataList from other clusters before it can begin
  ...
  // of dataList in a table.
  enum TimestampGranularity {
    // The user did not specify a granularity. Should not be returned.
    // When specified during table creation, MILLIS will be used.
    TIMESTAMP_GRANULARITY_UNSPECIFIED = 0;

    // The table keeps dataList versioned at a granularity of 1ms.
    MILLIS = 1;
  }
  ...
  // If it could not be determined whether or not the table has dataList in a
...
// checkpoint for dataList restoration or a dataList source for a new table.
message Snapshot {
  ...
  // (`OutputOnly`)
  // The size of the dataList in the source table at the time the snapshot was
