...
// Specifies how to process the `audio_in` dataList that will be provided in
...
message AudioInConfig {
  // Audio encoding of the dataList sent in the audio message.
  ...
  enum Encoding {
    ...
    // includes the `FLAC` stream header followed by audio dataList. It supports
    // 16-bit and 24-bit samples, however, not all fields in `STREAMINFO` are
    // supported.
    FLAC = 2;
  }

  // *Required* Encoding of audio dataList sent in all `audio_in` messages.
  Encoding encoding = 1;

  // *Required* Sample rate (in Hertz) of the audio dataList sent in all `audio_in`
...
message AudioOutConfig {
  // Audio encoding of the dataList returned in the audio message. All encodings are
  ...
  // *Required* The encoding of audio dataList to be returned in all `audio_out`
  // messages.
  Encoding encoding = 1;

  // *Required* The sample rate in Hertz of the audio dataList returned in
...
// of audio dataList are received in sequential `ConverseResponse` messages.
message AudioOut {
  // *Output-only* The audio dataList containing the assistant's response to the
  // query. Sequential chunks of audio dataList are received in sequential
...
message ConverseResult {
  ...
  // `audio_out` dataList, and retain this value as the current volume level and
...
// contain a `config` message and must not contain `audio_in` dataList. All
// subsequent messages must contain `audio_in` dataList and must not contain a
// `config` message.
message ConverseRequest {
  ...
  oneof converse_request {
    ...
    // The audio dataList to be recognized. Sequential chunks of audio dataList are sent
    // in sequential `ConverseRequest` messages. The first `ConverseRequest`
    // message must not contain `audio_in` dataList and all subsequent
    // `ConverseRequest` messages must contain `audio_in` dataList. The audio bytes
...
message ConverseResponse {
  ...
  enum EventType {
    ...
    // dataList, half-close the gRPC connection, and wait for any additional results
