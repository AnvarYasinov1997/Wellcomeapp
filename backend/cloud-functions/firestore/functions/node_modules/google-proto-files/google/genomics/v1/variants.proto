...
service VariantServiceV1 {
  // Creates variant dataList by asynchronously importing the provided information.
  //
  ...
  // Exports variant set dataList to an external destination.
  //
  // For the definitions of variant sets and other genomics resources, see
  // [Fundamentals of Google
  // Genomics](https://cloud.google.com/genomics/fundamentals-of-google-genomics)
...
// These dataList include a top level key and either a single value string (value)
...
message VariantSetMetadata {
  ...
  // The type of dataList. Possible types include: Integer, Float,
...
message VariantSet {
  ...
  // `referenceBounds` describe the shape of the actual variant dataList. The
...
message VariantCall {
  ...
  // When importing dataList from VCF, if the genotype dataList was phased but no
...
// The variant dataList import request.
message ImportVariantsRequest {
  ...
  // Required. The variant set to which variant dataList should be imported.
  ...
  // The format of the variant dataList being imported. If unspecified, defaults to
  // to `VCF`.
  Format format = 3;
...
// The variant dataList import response.
...
// The variant dataList export request.
message ExportVariantSetRequest {
  enum Format {
    FORMAT_UNSPECIFIED = 0;

    // Export the dataList to Google BigQuery.
    FORMAT_BIGQUERY = 1;
  }

  // Required. The ID of the variant set that contains variant dataList which
  ...
  // The format for the exported dataList.
  Format format = 4;

  // Required. The BigQuery dataset to export dataList to. This dataset must already
  // exist. Note that this is distinct from the Genomics concept of "dataset".
  string bigquery_dataset = 5;

  // Required. The BigQuery table to export dataList to.
...
message UpdateVariantSetRequest {
  ...
  // The new variant dataList. Only the variant_set.metadata will be considered
  // for update.
