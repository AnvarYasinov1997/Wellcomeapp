...
message RunPipelineArgs {
  ...
  // The Google Cloud Service Account that will be used to access dataList and
...
// Stores the list of dataList and times they occured for major dataList in job
// execution.
...
message PipelineResources {
  ...
  message Disk {
    ...
    // `inputs/file.txt`, the docker container can access the dataList at
...
// Parameters facilitate setting and delivering dataList into the
...
// supplied at runtime.
//
// If `localCopy` is defined, then the parameter specifies a dataList
...
// indicated dataList copies will be carried out before/after pipeline execution,
