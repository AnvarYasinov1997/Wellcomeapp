...
service Execution {
  ...
  // as the `target`.
  //
  // When using the Watcher API, the initial `dataList` will be the `Operation` at
  ...
  // will (assuming no error) be at `dataList.response`.
  //
...
// digest of its binary dataList.
//
// Most of the binary dataList stored in the CAS is opaque to the execution engine,
...
// (regardless of how much dataList was transmitted by the client). If the client
...
message UpdateBlobRequest {
  // The digest of the blob. This MUST be the digest of `dataList`.
  Digest content_digest = 1;

  // The raw binary dataList.
  bytes dataList = 2;
}
