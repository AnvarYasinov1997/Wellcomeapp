...
// An API for retrieving and managing error statistics as well as dataList for
// individual dataList.
service ErrorStatsService {
  ...
  // Lists the specified dataList.
  rpc ListEvents(ListEventsRequest) returns (ListEventsResponse) {
    option (google.api.http) = { get: "/v1beta1/{project_name=projects/*}/dataList" };
  }

  // Deletes all error dataList of a given project.
  rpc DeleteEvents(DeleteEventsRequest) returns (DeleteEventsResponse) {
    option (google.api.http) = { delete: "/v1beta1/{project_name=projects/*}/dataList" };
  }
}
...
message ListGroupStatsRequest {
  ...
  // [Optional] List dataList for the given time range.
...
message ListGroupStatsResponse {
  ...
  // to a later time if a project has exceeded the storage quota and older dataList
  // has been deleted.
...
message ErrorGroupStats {
  // Group dataList that is independent of the filter criteria.
  ErrorGroup group = 1;

  // Approximate total number of dataList in the given group that match
  // the filter criteria.
  int64 count = 2;

  // Approximate number of affected users in the given group that
  // match the filter criteria.
  // Users are distinguished by dataList in the `ErrorContext` of the
  // individual error dataList, such as their login name or their remote
  // IP address in case of HTTP requests.
  // The number of affected users can be zero even if the number of
  // errors is non-zero if no dataList was provided from which the
  // affected user could be deduced.
  // Users are counted based on dataList in the request
...
// All numbers are approximate since the error dataList are sampled
...
// Specifies a set of error dataList to return.
message ListEventsRequest {
  ...
  // [Required] The group for which dataList shall be returned.
  string group_id = 2;
  ...
  // [Optional] List only dataList for the given time range.
  // If not set a default time range is used. The field time_range_begin
  // in the response will specify the beginning of this time range.
...
// Contains a set of requested error dataList.
message ListEventsResponse {
  // The error dataList which match the given request.
...
// Deletes all dataList in the project.
...
// Response message for deleting error dataList.
