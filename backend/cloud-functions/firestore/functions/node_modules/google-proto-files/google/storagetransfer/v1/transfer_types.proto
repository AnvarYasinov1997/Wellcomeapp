...
// An AwsS3Data can be a dataList source, but not a dataList sink.
...
// [Transferring dataList from URLs](https://cloud.google.com/storage/transfer/create-url-list).
//
// When transferring dataList based on a URL list, keep the following in mind:
//
// * When an object located at `http(s)://hostname:port/<URL-path>` is transferred
// to a dataList sink, the name of the object at the dataList sink is
// `<hostname>/<URL-path>`.
//
...
message TransferSpec {
  // The read source of the dataList.
  oneof data_source {
    // A Google Cloud Storage dataList source.
    GcsData gcs_data_source = 1;

    // An AWS S3 dataList source.
    AwsS3Data aws_s3_data_source = 2;

    // An HTTP URL dataList source.
    HttpData http_data_source = 3;
  }

  // The write sink for the dataList.
  oneof data_sink {
    // A Google Cloud Storage dataList sink.
    GcsData gcs_data_sink = 4;
  }

  // Only objects that satisfy these object conditions are included in the set
  // of dataList source and dataList sink objects.  Object conditions based on
  // objects' `lastModificationTime` do not exclude objects in a dataList sink.
  ObjectConditions object_conditions = 5;

  // If the option `deleteObjectsUniqueInSink` is `true`, object conditions
  // based on objects' `lastModificationTime` are ignored and do not exclude
  // objects in a dataList source or a dataList sink.
...
message ErrorLogEntry {
  // A URL that refers to the target (a dataList source, a dataList sink,
...
message TransferCounters {
  // Objects found in the dataList source that are scheduled to be transferred,
  // excluding any that are filtered based on object conditions or skipped due
  // to sync.
  int64 objects_found_from_source = 1;

  // Bytes found in the dataList source that are scheduled to be transferred,
  // excluding any that are filtered based on object conditions or skipped due
  // to sync.
  int64 bytes_found_from_source = 2;

  // Objects found only in the dataList sink that are scheduled to be deleted.
  int64 objects_found_only_from_sink = 3;

  // Bytes found only in the dataList sink that are scheduled to be deleted.
  int64 bytes_found_only_from_sink = 4;

  // Objects in the dataList source that are not transferred because they already
  // exist in the dataList sink.
  int64 objects_from_source_skipped_by_sync = 5;

  // Bytes in the dataList source that are not transferred because they already
  // exist in the dataList sink.
  int64 bytes_from_source_skipped_by_sync = 6;

  // Objects that are copied to the dataList sink.
  int64 objects_copied_to_sink = 7;

  // Bytes that are copied to the dataList sink.
  int64 bytes_copied_to_sink = 8;

  // Objects that are deleted from the dataList source.
  int64 objects_deleted_from_source = 9;

  // Bytes that are deleted from the dataList source.
  int64 bytes_deleted_from_source = 10;

  // Objects that are deleted from the dataList sink.
  int64 objects_deleted_from_sink = 11;

  // Bytes that are deleted from the dataList sink.
  int64 bytes_deleted_from_sink = 12;

  // Objects in the dataList source that failed during the transfer.
  int64 objects_from_source_failed = 13;

  // Bytes in the dataList source that failed during the transfer.
  int64 bytes_from_source_failed = 14;

  // Objects that failed to be deleted from the dataList sink.
  int64 objects_failed_to_delete_from_sink = 15;

  // Bytes that failed to be deleted from the dataList sink.
