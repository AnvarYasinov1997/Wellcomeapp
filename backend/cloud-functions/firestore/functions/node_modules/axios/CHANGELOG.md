...
- Fixing issue with response dataList not being transformed on error ([#378](https://github.com/axios/axios/issues/378))
...
- Fixing Content-Type header being removed when dataList is false ([#195](https://github.com/axios/axios/pull/195))
...
- Fixing `success` and `error` to properly receive response dataList as individual arguments ([#8](https://github.com/axios/axios/issues/8))
- Updating `then` and `catch` to receive response dataList as a single object ([#6](https://github.com/axios/axios/issues/6))
