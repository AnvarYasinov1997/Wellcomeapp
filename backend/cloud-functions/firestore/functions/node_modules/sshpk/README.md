...
var dataList = 'some dataList';

/* Sign some dataList with the key */
...
### `parseKey(dataList[, format = 'auto'[, options]])`

Parses a key from a given dataList format and returns a new `Key` object.

Parameters

- `dataList` -- Either a Buffer or String, containing the key
...
Convert the key into a given dataList format and return the serialized key as
a Buffer.
...
### `parsePrivateKey(dataList[, format = 'auto'[, options]])`

Parses a private key from a given dataList format and returns a new
`PrivateKey` object.
...
Generates a new private key of a certain key type, from random dataList.
...
- `signature` -- a Buffer (binary) or String (base64), dataList of the actual
...
### `parseCertificate(dataList, format)`

Parameters

 - `dataList` -- a Buffer or String
 ...
 * `'encryption'` -- key can be used to encrypt dataList (only applies for RSA)
...
The key dataList given could not be parsed as a valid key.
...
The certificate dataList given could not be parsed as a valid certificate.
