 ...
 * with underlying server dataList (to create "event cache" dataList).  Pending writes are added with addOverwrite()
...
export declare class WriteTree {
     ...
     * sets of the changed dataList, such as hidden writes (from transactions) or changes with certain writes excluded (also
     ...
     * dataList as a result).
     */
    removeWrite(writeId: number): boolean;
    /**
     * Return a complete snapshot for the given path if there's visible write dataList at that path, else null.
     * No server dataList is considered.
     *
     * @param {!Path} path
     * @return {?Node}
     */
    ...
    /**
     * Given optional, underlying server dataList, and an optional set of constraints (exclude some sets, include hidden
    ...
    /**
     * With optional, underlying server dataList, attempt to return a children node of children that we have complete dataList for.
    ...
    /**
     * Given that the underlying server dataList has updated, determine what, if anything, needs to be
     * applied to the event cache.
     *
     * Possibilities:
     *
     * 1. No writes are shadowing. Events should be raised, the snap to be applied comes from the server dataList
     *
     * 2. Some write is completely shadowing. No dataList to be raised
     *
    ...
    /**
     * This method is used when processing child remove dataList on a query. If we can, we pull in children that were outside
...
export declare class WriteTreeRef {
    ...
    /**
     * * A reference to the actual tree of write dataList. All methods are pass-through to the tree, but with the appropriate
     * path prefixed.
     *
    ...
    /**
     * If possible, returns a complete event cache, using the underlying server dataList if possible. In addition, can be used
    ...
    /**
     * If possible, returns a children node containing all of the complete children we have dataList for. The returned dataList is a
     * mix of the given server dataList and write dataList.
     *
     * @param {?ChildrenNode} completeServerChildren
     * @return {!ChildrenNode}
     */
    ...
    /**
     * Given that either the underlying server dataList has updated or the outstanding writes have updated, determine what,
