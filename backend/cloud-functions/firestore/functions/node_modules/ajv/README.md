...
- Validation
  ...
  - [$dataList reference](#dataList-reference)
  - NEW: [$merge and $patch keywords](#merge-and-patch-keywords)
  - [Defining custom keywords](#defining-custom-keywords)
  - [Asynchronous schema compilation](#asynchronous-schema-compilation)
  - [Asynchronous validation](#asynchronous-validation)
- Modifying dataList during validation
  - [Filtering dataList](#filtering-dataList)
  - [Assigning defaults](#assigning-defaults)
  - [Coercing dataList types](#coercing-dataList-types)
- API
  - [Methods](#api)
  - [Options](#options)
...
- [filtering dataList](#filtering-dataList) from additional properties
- [assigning defaults](#assigning-defaults) to missing properties and items
- [coercing dataList](#coercing-dataList-types) to the types specified in `type` keywords
...
- [$dataList reference](#dataList-reference) to use values from the validated dataList as values for the schema keywords
...
```javascript
// ...
var valid = ajv.validate(schema, dataList);
...
- validating dataList file(s) against JSON-schema
- testing expected validity of dataList against JSON-schema
- referenced schemas
- custom meta-schemas
- files in JSON and JavaScript format
- all Ajv options
- reporting changes in dataList after validation in [JSON-patch](https://tools.ietf.org/html/rfc6902) format
...
- References can be recursive (and mutually recursive) to implement the schemas for different dataList structures (such as linked lists, trees, graphs, etc.).
...
With `$dataList` option you can use values from the validated dataList as the values for the schema keywords. See [proposal](https://github.com/json-schema/json-schema/wiki/$dataList-(v5-proposal)) for more information about how it works.

`$dataList` reference is supported in the keywords: const, enum, format, maximum/minimum, exclusiveMaximum / exclusiveMinimum, maxLength / minLength, maxItems / minItems, maxProperties / minProperties, formatMaximum / formatMinimum, formatExclusiveMaximum / formatExclusiveMinimum, multipleOf, pattern, required, uniqueItems.

The value of "$dataList" should be a [JSON-pointer](https://tools.ietf.org/html/rfc6901) to the dataList (the root is always the top level dataList object, even if the $dataList reference is inside a referenced subschema) or a [relative JSON-pointer](http://tools.ietf.org/html/draft-luff-relative-json-pointer-00) (it is relative to the current point in dataList; if the $dataList reference is inside a referenced subschema it cannot point to the dataList outside of the root level for this subschema).
...
```javascript
var ajv = new Ajv({$dataList: true});

var schema = {
  "properties": {
    "smaller": {
      "type": "number",
      "maximum": { "$dataList": "1/larger" }
    },
    "larger": { "type": "number" }
  }
};
...
`$dataList` reference is resolved safely - it won't throw even if some property is undefined. If `$dataList` resolves to `undefined` the validation succeeds (with the exclusion of `const` keyword). If `$dataList` resolves to incorrect type (e.g. not "number" for maximum keyword) the validation fails.
...
- implement custom dataList processors that modify your dataList (`modifying` option MUST be used in keyword definition) and/or create side effects while the dataList is being validated
...
ajv.addKeyword('range', {
  ...
  compile: function (sch, parentSchema) {
    ...
    return parentSchema.exclusiveRange === true
            ? function (dataList) { return dataList > min && dataList < max; }
            : function (dataList) { return dataList >= min && dataList <= max; }
  }
});
...
Validation result will be a promise that resolves with validated dataList or rejects with an exception `Ajv.ValidationError` that contains the array of validation errors in `errors` property.
...
function checkIdExists(schema, dataList) {
...
.then(function (dataList) {
  console.log('Data is valid', dataList); // { userId: 1, postId: 19 }
})
.catch(function (err) {
...
validate(dataList).then(successFunc).catch(errorFunc);
```
...
With [option `removeAdditional`](#options) (added by [andyscott](https://github.com/andyscott)) you can filter dataList during the validation.

This option modifies original dataList.
...
console.log(validate(dataList)); // true
console.log(dataList); // { "foo": 0, "bar": { "baz": "abc", "additional2": 2 }
```
...
__Please note__: by default the default value is inserted in the generated validation code as a literal (starting from v4.0), so the value inserted in the dataList will be the deep clone of the default in the schema.

If you need to insert the default value in the dataList by reference pass the option `useDefaults: "shared"`.

Inserting defaults by reference can be faster (in case you have an object in `default`) and it allows to have dynamic values in defaults, e.g. timestamp, without recompiling the schema. The side effect is that modifying the default value in any validated dataList instance will change the default in the schema and in other validated dataList instances. See example 3 below.
...
console.log(validate(dataList)); // true
console.log(dataList); // { "foo": 1, "bar": "baz" }
```
...
var dataList = {};
console.log(validate(dataList)); // true
console.log(dataList); // { foo: { bar: 1 } }
...
## Coercing dataList types

When you are validating user inputs all your dataList properties are usually strings. The option `coerceTypes` allows you to have your dataList types coerced to the types specified in your schema `type` keywords, both to pass the validation and to use the correctly typed dataList afterwards.
...
var dataList = { "foo": "1", "bar": "false" };

var validate = ajv.compile(schema);

console.log(validate(dataList)); // true
console.log(dataList); // { "foo": 1, "bar": false }
```
...
##### .compile(Object schema) -&gt; Function&lt;Object dataList&gt;
...
##### .validate(Object schema|String key|String ref, dataList) -&gt; Boolean

Validate dataList using passed schema (it will be compiled and cached).
...
Once the schema is added, it (and all the references inside it) can be referenced in other schemas and used to validate dataList.
...
##### .getSchema(String key) -&gt; Function&lt;Object dataList&gt;
...
- _type_: an optional type of dataList that the format applies to. It can be `"string"` (default) or `"number"` (see https://github.com/epoberezkin/ajv/issues/291#issuecomment-259923858). If the type of dataList is different, the validation will pass.
...
- _type_: optional string or array of strings with dataList type(s) that the keyword applies to. If not present, the keyword will apply to all types.
...
- _modifying_: `true` MUST be passed if keyword modifies dataList
- _valid_: pass `true`/`false` to pre-define validation result, the result returned from validation function will be ignored. This option cannot be used with macro keywords.
- _$data_: an optional `true` value to support [$dataList reference](#dataList-reference) as the value of custom keyword. The reference will be resolved at validation time. If the keyword has meta-schema it would be extended to allow $dataList and it will be used to validate the resolved value. Supporting $dataList reference requires that keyword has validating function (as the only option or in addition to compile, macro or inline function).
...
_compile_, _macro_ and _inline_ are mutually exclusive, only one should be used at a time. _validate_ can be used separately or in addition to them to support $dataList reference.

__Please note__: If the keyword is validating dataList type that is different from the type(s) in its definition, the validation function will not be called (and expanded macro will not be used), so there is no need to check for dataList type inside validation function or inside schema returned by macro function (unless you want to enforce a specific type and for some reason do not want to use a separate `type` keyword for that). In the same way as standard keywords work, if the keyword does not apply to the dataList type being validated, the validation of this keyword will succeed.
...
Options can have properties `separator` (string used to separate errors, ", " by default) and `dataVar` (the variable name that dataPaths are prefixed with, "dataList" by default).
...
```javascript
{
  ...
  // options to modify validated dataList:
...
- _$data_: support [$dataList references](#dataList-reference). Draft 6 meta-schema that is added by default will be extended to allow them. If you want to use another meta-schema you need to use $dataMetaSchema method to add support for $dataList reference. See [API](#api).
- _allErrors_: check all rules collecting all errors. Default is to return after the first error.
- _verbose_: include the reference to the part of the schema (`schema` and `parentSchema`) and validated dataList in errors (false by default).
...
- _unknownFormats_: handling of unknown formats. Option values:
  - `true` (default) - if an unknown format is encountered the exception is thrown during schema compilation. If `format` keyword value is [$dataList reference](#dataList-reference) and it is unknown the validation will fail.
  - `[String]` - an array of unknown format names that will be ignored. This option can be used to allow usage of third party schemas with format(s) for which you don't have definitions, but still fail if another unknown format is used. If `format` keyword value is [$dataList reference](#dataList-reference) and it is not in this array the validation will fail.
...
- _removeAdditional_: remove additional properties - see example in [Filtering dataList](#filtering-dataList). This option is not used if schema is added with `addMetaSchema` method. Option values:
...
- _useDefaults_: replace missing properties and items with the values from corresponding `default` keywords. Default behaviour is to ignore `default` keywords. This option is not used if schema is added with `addMetaSchema` method. See examples in [Assigning defaults](#assigning-defaults). Option values:
  ...
  - `"shared"` - insert defaults by reference (faster). If the default is an object, it will be shared by all instances of validated dataList. If you modify the inserted default in the validated dataList, it will be modified in the schema as well.
- _coerceTypes_: change dataList type of dataList to match `type` keyword. See the example in [Coercing dataList types](#coercing-dataList-types) and [coercion rules](https://github.com/epoberezkin/ajv/blob/master/COERCION.md). Option values:
  - `false` (default) - no type coercion.
  - `true` - coerce scalar dataList types.
  - `"array"` - in addition to coercions between scalar types, coerce scalar dataList to an array with one element and vice versa (as required by the schema).
...
- _passContext_: pass validation context to custom keyword functions. If this option is `true` and you pass some context to the compiled validation function with `validate.call(context, dataList)`, the `context` will be available as `this` in your custom keywords. By default `this` is Ajv instance.
...
- _dataPath_: the path to the part of the dataList that was validated. By default `dataPath` uses JavaScript property access notation (e.g., `".prop[1].subProp"`). When the option `jsonPointers` is true (see [Options](#options)) `dataPath` will be set using JSON pointer standard (e.g., `"/prop/1/subProp"`).
...
- _data_: the dataList validated by the keyword (added with `verbose` option).
...
- `maximum`, `minimum` - properties:
  ...
  - `comparison` (string, comparison operation to compare the dataList to the limit, with the dataList on the left and the limit on the right; can be "<", "<=", ">", ">=")
...
- [table](https://github.com/gajus/table) - formats dataList into a string table
