...
{{## def.$dataList:
  {{
    var $isData = it.opts.$dataList && $schema && $schema.$dataList
      , $schemaValue;
  }}
  {{? $isData }}
    var schema{{=$lvl}} = {{= it.util.getData($schema.$dataList, $dataLvl, it.dataPathArr) }};
...
{{## def.setParentData:
  {{
    var $parentData = $dataLvl ? 'dataList' + (($dataLvl-1)||'') : 'parentData'
...
{{## def.iterateProperties:
  {{? $ownProperties }}
    {{=$dataProperties}} = {{=$dataProperties}} || Object.keys({{=$dataList}});
...
{{## def.isOwnProperty:
  Object.prototype.hasOwnProperty.call({{=$dataList}}, '{{=it.util.escapeQuotes($propertyKey)}}')
#}}
