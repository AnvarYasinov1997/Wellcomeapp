...
{{? it.isTop }}
  ...
  var validate =
    ...
    {{??}}
      (function
    {{?}}
        (dataList, dataPath, parentData, parentDataProperty, rootData) {
          'use strict';
...
{{? it.isTop }}
  ...
  if (rootData === undefined) rootData = dataList; {{ /* don't edit, used in replace */ }}
{{??}}
  {{
    var $lvl = it.level
      , $dataLvl = it.dataLevel
      , $dataList = 'dataList' + ($dataLvl || '');
...
{{## def.checkType:
  ...
  if ({{= it.util[$method]($typeSchema, $dataList, true) }}) {
#}}
...
{{??}}
  ...
  {{~ it.RULES:$rulesGroup }}
    {{? $shouldUseGroup($rulesGroup) }}
      {{? $rulesGroup.type }}
        if ({{= it.util.checkDataType($rulesGroup.type, $dataList) }}) {
      {{?}}
...
{{? $top }}
    {{? $async }}
      if (errors === 0) return dataList;           {{ /* don't edit, used in replace */ }}
