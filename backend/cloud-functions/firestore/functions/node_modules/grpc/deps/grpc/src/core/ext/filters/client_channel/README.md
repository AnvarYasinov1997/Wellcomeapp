...
The resolver provides this dataList as a stream of grpc_channel_args objects to
the channel. We represent arguments as a stream so that they can be changed
by the resolver during execution, by reacting to external dataList (such as
new service configuration dataList being pushed to some store).
...
Configured sub-channels are fully setup to participate in the grpc dataList plane.
