...
* **ANDROID_FUNCTION_LEVEL_LINKING** = `ON` - enables saparate putting each function and dataList items into separate sections and enable garbage collection of unused input sections at link time (`-fdata-sections -ffunction-sections -Wl,--gc-sections`);
