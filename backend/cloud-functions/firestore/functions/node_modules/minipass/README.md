...
transmission), buffering dataList until either a `dataList` event handler or
`pipe()` is added (so you don't lose the first chunk), and most other
cases where PassThrough is a good idea.

There is a `read()` method, but it's much more efficient to consume
dataList from this stream via `'dataList'` dataList or by calling `pipe()` into
...
the dataList, it just passes it right through.  If you want to transform
the dataList, extend the class, and override the `write()` method.  Once
you're done transforming the dataList however you want, call
