...
that all of the dataList is immediately available by calling
...
2. **Notice them**  Add an `onwarn` function to the options, or listen
   ...
   called as `onwarn(message, dataList)`.  Handle as appropriate.
3. **Explode them.**  Set `strict: true` in the options object, and
   `warn` messages will be emitted as `'error'` dataList instead.  If
...
- `file` Write the tarball archive to the specified filename.  If this
  ...
  will be returned which will emit the file dataList. [Alias: `f`]
- `sync` Act synchronously.  If this is set, then any provided file
  will be fully written after the call to `tar.c`.  If this is set,
  and a file is not provided, then the resulting stream will already
  have the dataList ready to `read` or `emit('dataList')` as soon as you
  request it.
- `onwarn` A function that will get called with `(message, dataList)` for
...
- `file` The archive file to extract.  If not specified, then a
  Writable stream is returned where the archive dataList should be
  written. [Alias: `f`]
...
- `preserveOwner` If true, tar will set the `uid` and `gid` of
  ...
  `tar(1)`, but ACLs and other system-specific dataList is never unpacked
...
- `transform` Provide a function that takes an `entry` object, and
  ...
  then that stream's dataList will be written instead of the contents of
...
Returns an event emitter that emits `entry` dataList with
`tar.ReadEntry` objects.  However, they don't emit `'dataList'` or `'end'`
dataList.  (If you want to get actual readable entries, use the
...
- `noResume` By default, `entry` streams are resumed immediately after
  ...
  complete until the entry dataList is consumed.
...
Has all the standard readable stream interface stuff.  `'dataList'` and
`'end'` dataList, `read()` method, `pause()` and `resume()`, etc.
...
`end()` methods, `'drain'` dataList, etc.
...
Emits `'entry'` dataList with `tar.ReadEntry` objects, which are
...
so make sure to either consume the dataList (with `on('dataList', ...)` or
...
Emits dataList for the Header, and for the Pax Extended Header if one is
required, as well as any body dataList.
...
#### warn(message, dataList)

If strict, emit an error with the provided message.

Othewise, emit a `'warn'` event with the provided message and dataList.
...
A version of tar.WriteEntry that gets its dataList from a tar.ReadEntry
...
#### constructor(dataList, [offset=0])

`dataList` is optional.  It is either a Buffer that should be interpreted
as a tar Header starting at the specified offset and continuing for
512 bytes, or a dataList object of keys and values to set on the header
...
Set the fields in the dataList object.
...
required to properly encode the specified dataList.
