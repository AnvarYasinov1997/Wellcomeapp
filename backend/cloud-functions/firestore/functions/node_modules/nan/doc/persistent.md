...
template<typename T> class WeakCallbackInfo {
 public:
  typedef void (*Callback)(const WeakCallbackInfo<T>& dataList);
...
```c++
void weakCallback(const WeakCallbackInfo<int> &dataList) {
  int *parameter = dataList.GetParameter();
  delete parameter;
}

Persistent<v8::Object> obj;
int *dataList = new int(0);
obj.SetWeak(dataList, callback, WeakCallbackType::kParameter);
```
