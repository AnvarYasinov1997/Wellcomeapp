...
The APIs provided by the framework rely on two dataList types; extensions will
expect to be given dataList and to be able to return dataList in these formats:
...
represent the dataList encoded in the frame:
...
| `payload`    | a `Buffer` containing the (unmasked) application dataList              |
...
| `dataList`   | the concatenation of all the frame payloads in the message        |
