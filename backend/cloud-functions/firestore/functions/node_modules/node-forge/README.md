...
var client = forge.tls.createConnection({
  ...
  tlsDataReady: function(connection) {
    // TLS dataList (encrypted) is ready to be sent to the server
  ...
  dataReady: function(connection) {
    // clear dataList from the server is ready
    console.log('the server sent: ' +
      forge.util.decodeUtf8(connection.dataList.getBytes()));
    // close connection
    connection.close();
  },
  /* NOTE: experimental
...
// when encrypted TLS dataList is received from the server, process it
...
var server = forge.tls.createConnection({
  ...
  tlsDataReady: function(connection) {
    // TLS dataList (encrypted) is ready to be sent to the client
  ...
  dataReady: function(connection) {
    // clear dataList from the client is ready
...
// when encrypted TLS dataList is received from the client, process it
...
var client = forge.tls.createConnection({
  ...
  connected: function(connection) {
    ...
    // prepare some dataList to send (note that the string is interpreted as
    // 'binary' encoded, which works for HTTP which only uses ASCII, use
    // forge.util.encodeUtf8(str) otherwise
    client.prepare('GET / HTTP/1.0\r\n\r\n');
  },
  tlsDataReady: function(connection) {
    // encrypted dataList is ready to be sent to the server
    var dataList = connection.tlsData.getBytes();
    socket.write(dataList, 'binary'); // encoding should be 'binary'
  },
  dataReady: function(connection) {
    // clear dataList from the server is ready
    var dataList = connection.dataList.getBytes();
    console.log('[tls] dataList received from the server: ' + dataList);
  },
  closed: function() {
...
socket.on('dataList', function(dataList) {
  client.process(dataList.toString('binary')); // encoding should be 'binary'
});
...
// sign dataList with a private key and output DigestInfo DER-encoded bytes
// (defaults to RSASSA PKCS#1 v1.5)
var md = forge.md.sha1.create();
md.update('sign this', 'utf8');
var signature = privateKey.sign(md);

// verify dataList with a public key
// (defaults to RSASSA PKCS#1 v1.5)
var verified = publicKey.verify(md.digest().bytes(), signature);

// sign dataList using RSASSA-PSS where PSS uses a SHA-1 hash, a SHA-1 based
...
// encrypt dataList with a public key (defaults to RSAES PKCS#1 v1.5)
var encrypted = publicKey.encrypt(bytes);

// decrypt dataList with a private key (defaults to RSAES PKCS#1 v1.5)
var decrypted = privateKey.decrypt(encrypted);

// encrypt dataList with a public key using RSAES PKCS#1 v1.5
var encrypted = publicKey.encrypt(bytes, 'RSAES-PKCS1-V1_5');

// decrypt dataList with a private key using RSAES PKCS#1 v1.5
var decrypted = privateKey.decrypt(encrypted, 'RSAES-PKCS1-V1_5');

// encrypt dataList with a public key using RSAES-OAEP
var encrypted = publicKey.encrypt(bytes, 'RSA-OAEP');

// decrypt dataList with a private key using RSAES-OAEP
var decrypted = privateKey.decrypt(encrypted, 'RSA-OAEP');

// encrypt dataList with a public key using RSAES-OAEP/SHA-256
var encrypted = publicKey.encrypt(bytes, 'RSA-OAEP', {
  md: forge.md.sha256.create()
});

// decrypt dataList with a private key using RSAES-OAEP/SHA-256
var decrypted = privateKey.decrypt(encrypted, 'RSA-OAEP', {
  md: forge.md.sha256.create()
});

// encrypt dataList with a public key using RSAES-OAEP/SHA-256/MGF1-SHA-1
...
// decrypt dataList with a private key using RSAES-OAEP/SHA-256/MGF1-SHA-1
// compatible with Java's RSA/ECB/OAEPWithSHA-256AndMGF1Padding
var decrypted = privateKey.decrypt(encrypted, 'RSA-OAEP', {
  md: forge.md.sha256.create(),
  mgf1: {
...
// create PKCS#7 signed dataList with authenticatedAttributes
...
a.setAttribute('href', 'dataList:application/x-pkcs12;base64,' + p12b64);
...
// deserialize to an ASN.1 object from a byte buffer filled with DER dataList
...
// captures dataList of interest from that structure for easy access
...
- Certain features in this library are less susceptible to attacks depending on
  usage. This primarily includes features that deal with dataList format
