...
to stream dataList in and out of it without needing to know anything about how the
...
server.on('upgrade', function(request, socket, body) {
  ...
  driver.messages.on('dataList', function(message) {
...
cannot write dataList to it, it only tells you what request dataList we parsed from the
input.
...
process incoming dataList and send outgoing dataList.
...
Client drivers have two additional properties for reading the HTTP dataList that was
...
Note that most of these methods are commands: if they produce dataList that should
be sent over the socket, they will give this to you by emitting `dataList` dataList on
...
`dataList` attribute containing either a string in the case of a text message or a
`Buffer` in the case of a binary message.

You can also listen for messages using the `driver.messages.on('dataList')` event,
...
Takes a string and parses it, potentially resulting in message dataList being
emitted (see `on('message')` above) or in dataList being sent to `driver.io`.  You
should send all dataList you receive via I/O to this method by piping a stream into
`driver.io`.
