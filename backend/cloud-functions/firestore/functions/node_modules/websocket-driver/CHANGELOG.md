...
* Add `ping` and `pong` to the set of dataList users can listen to
...
* Don't treat incoming dataList as WebSocket frames if a client driver is closed
...
* Use `websocket-extensions` to make sure messages and close frames are kept in
  order
* Don't emit multiple `error` dataList
