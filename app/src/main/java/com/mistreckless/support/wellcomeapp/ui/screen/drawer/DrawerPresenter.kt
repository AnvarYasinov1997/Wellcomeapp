package com.mistreckless.support.wellcomeapp.ui.screen.drawer

import com.mistreckless.support.wellcomeapp.ui.BasePresenter
import com.mistreckless.support.wellcomeapp.ui.BasePresenterProviderFactory
import com.mistreckless.support.wellcomeapp.ui.MainActivity
import com.mistreckless.support.wellcomeapp.ui.model.HeaderItem
import com.mistreckless.support.wellcomeapp.ui.model.ListItem
import com.mistreckless.support.wellcomeapp.ui.presenterHolder
import javax.inject.Inject

/**
 * Created by @mistreckless on 27.08.2017. !
 */

class DrawerPresenter : BasePresenter<DrawerView, MainActivity>() {
    override fun onFirstViewAttached() {
        val drawerItems = List(2,{
            if (it==0) HeaderItem()
            else ListItem("Wall", android.R.drawable.ic_menu_send)
        })
        getView()?.initUi(drawerItems)
    }

    companion object {
        const val TAG = "DrawerPresenter"
    }
}

class DrawerPresenterProviderFactory @Inject constructor() : BasePresenterProviderFactory<DrawerPresenter>{
    override fun get(): DrawerPresenter {
        if (presenterHolder.contains(DrawerPresenter.TAG))
            return presenterHolder[DrawerPresenter.TAG] as DrawerPresenter
        else{
            val presenter = DrawerPresenter()
            presenterHolder.put(DrawerPresenter.TAG,presenter)
            return presenter
        }
    }
}