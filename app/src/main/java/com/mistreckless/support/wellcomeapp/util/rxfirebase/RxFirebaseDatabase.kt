...
class RxSetValueDep<out T>(val ref: DatabaseReference, val value: T) : CompletableOnSubscribe {
...
class RxSingleQueryDep<T>(val query: Query, val clazz: Class<T>) : SingleOnSubscribe<T> {
...
class RxObserveValueEventDep<T>(val ref: DatabaseReference, val clazz: Class<T>) : ObservableOnSubscribe<T> {
